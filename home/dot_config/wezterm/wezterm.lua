...
-- Color scheme (custom themes registered via appearance module; see themes/)
config.color_schemes = appearance.get_color_schemes()
config.color_scheme = appearance.get_color_scheme()
...
-- Hide tab bar when only one tab is open (keeps native title bar + window buttons)
...
elseif is_windows() then
  config.default_prog = { wezterm.home_dir .. "\\scoop\\shims\\pwsh.exe", "-NoLogo" }
end
