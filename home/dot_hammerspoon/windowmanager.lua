...
local chain = function(gridPositions)
  ...
  return function()
    ...
    if lastSeenChain ~= gridPositions or lastSeenAt < now - chainResetInterval or lastSeenWindow ~= id then
      sequenceNumber = 1
...
local chainSideBySide = function(gridPositions)
  ...
  return function()
    ...
    if
      ...
      or lastSeenAtSideBySide < now - chainResetInterval
      or lastSeenWindowSideBySide ~= id
    then
      sequenceNumber = 1
      lastSeenSideBySide = gridPositions
    end
