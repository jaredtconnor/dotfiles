...
$RepoUrl = if ($env:DOTFILES_REPO_URL) { $env:DOTFILES_REPO_URL } else { "https://github.com/jaredtconnor/.dotfiles.git" }
...
if (Test-Path $dotbotConf) {
    ...
    # .config subdirs
    @("nvim", "powershell", "ghostty", "Code/User", "zed") | ForEach-Object {
...
if ((Test-Path (Join-Path $DotfilesDir ".git")) -and -not (Test-Path (Join-Path $DotfilesDir "install.conf.yaml"))) {
    ...
    Write-Info "Cloning $RepoUrl to $DotfilesDir..."
